...
public class ArticleServiceImpl implements ArticleService {
  ...
  public List<Article> userList(String userName) {
    ...
    if (hasKey != null && hasKey) {
      ...
      if (members.size() <= 2) {
        ...
        members.stream().sorted((x, y) -> x.getTop() > y.getTop() ? 1 : 0)
            .forEach(list::add);
      }
    }
    ...
    // 筛选出已被删除得文章，删除为逻辑删除，以后可考虑文章找回功能
    return list.stream()
        .filter(article -> !article.getStatus().equals(ArticleStatus.DELETE.getCode()))
  ...
  public Article get(String userName, String principal) {
    ...
    log.info(
        "cn.pzhu.forum.service.impl.ArticleServiceImpl.get(java.lang.String, java.lang.String)-" +
    ...
    if (hasKey != null && hasKey) {
      ...
      Stream<Article> limit = members.stream()
          .filter((x) -> x.getPrincipal().equals(principal) && x.getUserName().equals(userName))
          .limit(1);
  ...
  public boolean updateArticleStatus(int id, ArticleStatus status) {
    ...
    return articleDao.updateArticleStatus(id, status.getCode()) > 0;
  }
  ...
  @Override
  public List<Article> selectArticleByKeywordOfPending(String text, Integer start, int limit) {
    text = "%"+text +"%";
    return articleDao.listWithPageFroAdminPending(text,start,limit);
  }
  ...
  private List<Article> articlesList(ArticleType articleType, int number) {
    ...
    do {
      ...
      for (int i = 0; i < list.size(); i++) {
        ...
        if (idList.size() > 0 && idList.contains(sort.getRely()) && !idList
            .contains(sort.getId())) {
          idList.add(sort.getId());
        }
      }
  ...
  /**
   * 比较两个字符串的相识度 核心算法：用一个二维数组记录每个字符串是否相同，如果相同记为0，不相同记为1，每行每列相同个数累加 则数组最后一个数为不相同的总数，从而判断这两个字符的相识度
   * <p>
   * 设 s 的长度为 n，t 的长度为 m。如果 n = 0，则返回 m 并退出；如果 m=0，则返回 n 并退出。否则构建一个数组 d[0..m, 0..n]。 将第0行初始化为
   * 0..n，第0列初始化为0..m。 依次检查 s 的每个字母(i=1..n)。 依次检查 t 的每个字母(j=1..m)。 如果 s[i]=t[j]，则 cost=0；如果
   * s[i]!=t[j]，则 cost=1。将 d[i,j] 设置为以下三个值中的最小值： 紧邻当前格上方的格的值加一，即 d[i-1,j]+1 紧邻当前格左方的格的值加一，即
   * d[i,j-1]+1 当前格左上方的格的值加cost，即 d[i-1,j-1]+cost 重复3-6步直到循环结束。d[n,m]即为莱茵斯坦距离。
   *
   * @param str 源字符串
   * @param target 目标字符串
   * @return 相似度
